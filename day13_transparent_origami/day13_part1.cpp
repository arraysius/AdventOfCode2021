...
void puzzle() {
    ...
    char foldDirection;
    int foldPosition;
    inputFile >> line >> line >> foldDirection >> _ >> foldPosition;

    vector<vector<char>> foldedPaper;
    if (foldDirection == 'y') {
        // Prepare folded paper
        foldedPaper = vector<vector<char>>(foldPosition,
                                           vector<char>(paper[0].size(), '.'));
        ...
        // Prepare folded paper
        foldedPaper =
            vector<vector<char>>(paper.size(), vector<char>(foldPosition, '.'));
        ...
        // Fold paper
        for (int row = 0; row < paper.size(); row++) {
            for (int col = foldPosition + 1; col < paper[row].size(); col++) {
